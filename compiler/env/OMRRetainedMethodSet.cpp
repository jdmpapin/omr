...
void OMR::RetainedMethodSet::keepalive() { flatten(FlattenMode_Keepalive); }

void OMR::RetainedMethodSet::bond() { flatten(FlattenMode_Bond); }

void OMR::RetainedMethodSet::flatten(FlattenMode mode)
{
    ...
    if (_comp->getOption(TR_TraceRetainedMethods)) {
        ...
        traceMsg(_comp, "RetainedMethodSet %p: %s %p %.*s.%.*s%.*s\n", this, flattenKindName,
            _method->getNonPersistentIdentifier(), _method->classNameLength(), _method->classNameChars(),
            _method->nameLength(), _method->nameChars(), _method->signatureLength(), _method->signatureChars());
    }
    ...
    if (!_parent->willRemainLoaded(_method)) {
        ...
        if (_comp->getOption(TR_TraceRetainedMethods)) {
            traceMsg(_comp, "RetainedMethodSet %p: added %s method %p (key %p)\n", _parent, flattenKindName,
                _method->getNonPersistentIdentifier(), key);
        }
    }
    ...
    if (mode == FlattenMode_Bond) {
        ...
        mergeIntoParent();
        TR_ASSERT_FATAL(_parent->willRemainLoaded(_method), "method should now be guaranteed to remain loaded");
    }
...
void OMR::RetainedMethodSet::mergeIntoParent()
{
...
void OMR::RetainedMethodSet::propagateMethods(MethodMap &dest, const MethodMap &src)
{
