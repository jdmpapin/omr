...
template<class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::ClearLastOneIfThereIsOneInRange(SparseBitIndex low,
    SparseBitIndex high, bool &foundone)
{
    if (IsZero()) {
        foundone = false;
...
template<class Allocator>
inline void ASparseBitVector<Allocator>::GrowSegment(typename ASparseBitVector<Allocator>::Segment &thisSegment,
    uint32_t size, uint32_t numValues)
{
    if (numValues > 0) {
        if (numValues <= 1024) {
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::GetSegment(typename ASparseBitVector<Allocator>::Segment &thisSegment,
...
template<class Allocator>
inline typename ASparseBitVector<Allocator>::Segment *ASparseBitVector<Allocator>::OrSegment(SparseBitIndex index,
    SparseBitIndex count, uint16_t *bits)
{
...
template<class Allocator>
template<class str>
inline void ASparseBitVector<Allocator>::DumpSegment(str &out,
...
template<class Allocator> inline void ASparseBitVector<Allocator>::RemoveSegmentAt(SparseBitIndex segmentIndex)
{
    ...
    if (fNumberOfSegments > 1) {
        ...
        removedSegment.deallocate(*this);
        fNumberOfSegments -= 1;
    } else {
        fBase[0].deallocate(*this);
